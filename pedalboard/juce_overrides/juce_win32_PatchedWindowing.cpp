...
public:
    ...
    double getPlatformScaleFactor() const noexcept override
    {
        ...
        // Patched for pedalboard (mstand fork): only a child window takes the
        // scale of the window it sits in. For a top-level window GetParent()
        // returns its owner, and an owned window on another monitor would
        // take the owner's scale instead of its own.
        const auto isChild = (GetWindowLongPtr (hwnd, GWL_STYLE) & WS_CHILD) != 0;

        if (auto* parentHWND = isChild ? GetParent (hwnd) : nullptr)
        {
